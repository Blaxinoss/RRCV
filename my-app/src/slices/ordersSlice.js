...
const fetchOrders = createAsyncThunk('orders/fetchOrders', async () => {
    const response = await axios.get('https://rrccserver.vercel.app/api/orders'); // Fetch orders from the API
    return response.data; // Return the fetched data
});
// Create async thunk for adding an order
const addOrder = createAsyncThunk('orders/addOrder', async (orderData) => {
    const response = await axios.post('https://rrccserver.vercel.app/api/orders/add', orderData); // Post new order to the API
    return response.data; // Return the newly added order
});

export const callOrder = createAsyncThunk('orders/callOrder', async (orderId) => {
    const response = await axios.patch(`https://rrccserver.vercel.app/api/orders/${orderId}`, { status: 'Called' });
...
const acceptOrder = createAsyncThunk('orders/acceptOrder', async (orderId) => {
    const response = await axios.put(`https://rrccserver.vercel.app/api/orders/accept/${orderId}`); // Accept the order in the API
...
const deleteOrder = createAsyncThunk('orders/deleteOrder', async (orderId) => {
    await axios.delete(`https://rrccserver.vercel.app/api/orders/${orderId}`); // Delete the order by ID
