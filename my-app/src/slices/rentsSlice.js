...
const fetchRents = createAsyncThunk('rents/fetchRents', async () => {
    const response = await axios.get('https://rrccserver.vercel.app/api/rents');
    return response.data;
});

// Async thunk to add a new rent
const addRent = createAsyncThunk('rents/addRent', async (rentData, { dispatch }) => {
    const response = await axios.post('https://rrccserver.vercel.app/api/rents/add', rentData);
...
const deleteRent = createAsyncThunk('rents/deleteRent', async (rentId) => {
    await axios.delete(`https://rrccserver.vercel.app/api/rents/delete/${rentId}`);
...
const updateRentStatus = createAsyncThunk('rents/updateRentStatus', async ({ rentId, newStatus }) => {
    const response = await axios.put(`https://rrccserver.vercel.app/api/rents/updateStatus/${rentId}`, { status: newStatus });
...
const updateRent = createAsyncThunk('rents/updateRent', async (rentData) => {
    ...
    const response = await axios.put(`https://rrccserver.vercel.app/api/rents/update/${_id}`, updates);
    return response.data; // Return updated rent
});
